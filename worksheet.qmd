...
```{r}
unit <- str_extract(energy_sources, "\\w+Wh" )
```

c.  Clean the names to keep only the energy source name, trimming whitespace and removing parentheses.

```{r}
energy_sources <- str_replace(energy_sources, "Solar (MWh)", "Solar")
energy_sources <- str_replace(energy_sources, "Wind (GWh)", "Wind")
energy_sources <- str_replace(energy_sources, "Coal (MWh)", "Coal")
energy_sources <- str_replace(energy_sources, "Hydro (kWh)", "Hydro")
```
...
energy_clean <- tibble(energy_sources, unit)
